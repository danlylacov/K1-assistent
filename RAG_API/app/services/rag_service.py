...
class RAGService:
    ...
    def initialize(self):
        ...
        # Ленивая загрузка модели - не загружаем при старте для экономии памяти
        # Модель загрузится автоматически при первом запросе
        logger.info("ℹ️  Модель embedding будет загружена при первом запросе (ленивая загрузка)")
